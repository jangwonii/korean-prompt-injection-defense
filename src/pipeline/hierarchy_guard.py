...
class HierarchyGuard:
    ...
    def analyze(
        ...
        if re.search(r"시스템\s*프롬프트|system\s*prompt", text, flags=re.IGNORECASE) or "시스템프롬프트" in compact:
            score += 45
            violated_level = "SYSTEM"
            evidence.append("hierarchy: user requested system-level context")
        if re.search(r"개발자\s*메시지|developer\s*message|내부\s*규칙|hidden\s*instruction", text, flags=re.IGNORECASE) or any(
            term in compact for term in ["개발자메시지", "내부규칙", "숨겨진지시"]
        ):
            score += 40
